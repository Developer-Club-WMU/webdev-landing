import { pipelineDeals, pipelineStages } from "@/app/(officer)/officer/pipeline/pipeline.config";
...
export function usePipelineContainerViewModel(pipelineId?: string) {
    ...
    useEffect(() => {
        if (pipelineData && pipelineId && leadsData) {
            ...
            const convertedDeals: PipelineCellProps[] = leadsData.map((lead: LeadFromDB) => ({
                ID: lead.id,
                title: lead.title,
                description: lead.description ?? "",
                capital: lead.capitalValue ? `$${lead.capitalValue.toLocaleString()}` : "$0",
    ...
    const onCardDrop = (dealId: string, newStage: LeadStatus) => {
        ...
        setDeals((prev: PipelineCellProps[]) => prev.map((deal) => (deal.ID === dealId ? { ...deal, status: newStage } : deal)));

        // If we have pipeline data, find the target segment and update in database
        if (pipelineData && pipelineId) {
            const targetSegment = pipelineData.segments.find((segment) => segment.name === newStage);
    ...
    const moveLead = api.crm.leads.moveToSegment.useMutation({
        onError: (error) => {
            ...
            if (leadsData) {
                const originalDeals: PipelineCellProps[] = leadsData.map((lead: LeadFromDB) => ({
                    ID: lead.id,
                    title: lead.title,
                    description: lead.description ?? "",
                    capital: lead.capitalValue ? `$${lead.capitalValue.toLocaleString()}` : "$0",
    ...
    const viewModels: KanbanColumnViewModel[] =
        pipelineId && pipelineData?.segments
            ? pipelineData.segments.map((segment) => {
                  const filteredDeals = deals.filter((deal) => deal.status === (segment.name as LeadStatus));
                  const header = {
                      title: segment.name,
                      totalItems: filteredDeals.length,
                      control: {
                          onClick: () => console.log(`Clicked ${segment.name}!`),
                          icon: "📋",
                      },
                      color: "#E0F2FE",
                  };
                  return new KanbanColumnViewModel(segment.name as LeadStatus, header, deals, segment.id);
              })
            : pipelineStages.map((stage) => {
                  ...
                  return new KanbanColumnViewModel(stage.status, header, deals, undefined);
              });
